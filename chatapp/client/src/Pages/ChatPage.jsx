...
const fetchChannelMessages = async (channelName) => {
  ...
  const handleQuoteMessage = (msg) => {
    // Set the quoted message state with the message's sender and content
    setQuotedMessage({
      sender: msg.username || msg.sender,  // Use either username or sender (based on your data)
      text: msg.chat_content,
    });

    // Add the quoted message to the new message input (formatting it properly)
    setNewMessage(`> ${msg.username || msg.sender}: ${msg.chat_content}\n\n`); // Add sender name for clarity
  };
  const handleDeleteMessage = async (messageId) => {
    if (userRole !== "Admin") return;  // Check if the user is an admin
